%% Lagrange11D
% Class that implements Lagrange-1 elements in 2D.

%%Copy style%
%handle

%%Inheritance
% lagrange11D < finiteElements1D < finiteElements < handle

classdef Lagrange11D < finiteElements1D
    properties(Constant = true)
        %% Constant properties with Access = public
        %
        % * S       (double) *vector* that store the element stiffness matrices.
        % * C       (double) *vector* that store the element gradient matrices
        % * M       (double) *vector* that stores the element mass matrix
        % * F       (double) vector that stores the source mass vector.
        % * idx     (double) vector that stores order of Points in element field.

        % stiff element
        S = reshape([1 -1;...
                    -1 1],4,1);
        % mass element
        M = reshape(1/6*[2 1;...
                         1 2],4,1);
        % convection
        C = reshape([-1/2 1/2;...
                     -1/2 1/2],4,1);
        % RHS element
        F = 0.5*[1 1]';

        % index of neighbours
        % trivial but for generality.
        idx = 1:2;
        assembleStep = 1;
    end

##     methods
##        function obj = Lagrange11D()
##        end
##
##        function [lengths, areas] = sideLengthAndArea(obj)
##            % Implementation of sideLengthAndArea method
##            lengths = zeros(obj.nElements, 1);
##            areas = zeros(obj.nElements, 1);
##            for i = 1:obj.nElements
##                % Assuming grid is a property of the Lagrange11D object
##                edge_length = norm(obj.grid.p(:, obj.grid.t(1, i)) - obj.grid.p(:, obj.grid.t(2, i)));
##                element_area = 0.5 * edge_length; % Assuming 1D, adjust as needed for 2D
##                lengths(i) = edge_length;
##                areas(i) = element_area;
##            end
##        end
##
##        function J = makeJ(obj, grid)
##            % Implementation of makeJ method
##            % Assuming grid is a property of the Lagrange11D object
##            J = zeros(obj.nElements, 1);
##            for i = 1:obj.nElements
##                % Assuming grid is a property of the Lagrange11D object
##                J(i) = det([obj.grid.p(:, obj.grid.t(2, i)) - obj.grid.p(:, obj.grid.t(1, i))]);
##            end
##        end
##
##        function c_centered = point2Center(obj, c)
##            % Implementation of point2Center method
##            % Assuming c is a vector of point coordinates
##            c_centered = (c(obj.grid.t(1, :)) + c(obj.grid.t(2, :))) / 2;
##        end
##
##    end

    % Must be public since it will be used in lagrange12D for assemble the
    % boundary matrix.
    methods(Static,Access = public)
        %% Overwritten methods
        %
        % * makeIndex
        % * fluxThroughEdges
        % * fluxJumps
        %
        function [idxvec0,idxvec1,idxvec2] = makeIndex(idx,np)
            idxvec0 = reshape(idx,1,np*2);
            idxvec1 = reshape([idx;idx],1,np*4);
            idxvec2 =  reshape([idx(1,:);idx(1,:);...
                                idx(2,:);idx(2,:)]...
                                                  ,1,4*np);
        end
    end

    methods(Static,Access = protected)
        function fluxThrougElementEdges= fluxThroughEdges(obj,u,c)
            % obj.fluxThroughEdges(grid,u,c)
            % Static method.
            % Arguments check:
            switch length(c)
                case {1, obj.nElements}
                    % c fits!
                case obj.nPoints
                    c = obj.point2Center(c);
                otherwise
                    % c is miss-dimensioned
                    obj.wrongFormat.throwAsCaller();
            end
            if length(u)~=obj.nPoints
                obj.wrongFormat.throwAsCaller();
            end
            % elementArea is in 1D the same as dx!
            [~, elementArea] = obj.sideLengthAndArea;
            % 2 x nElements matrix, rather formal...

            fluxThrougElementEdges = [1;-1]*...
                (c.*(u(obj.t(2,:))-u(obj.t(1,:)))'./elementArea);
        end


        function jumps = fluxJumps(obj,fluxThroughElementEdges,~)
            % --- this is dimension depending -- here 1D sub intervals
            interiorEdges=sparse(obj.t([1 2],:),...
                obj.t([1 2],:),1,obj.nPoints,obj.nPoints);

            % intj+intj.' is 2 if interior edge and 1 if exterior edge
            interiorEdges=round(interiorEdges/3);
            jmps = sparse(obj.t([1 2],:),obj.t([1 2],:),...
                fluxThroughElementEdges([1 2],:),obj.nPoints,obj.nPoints);
            jmps = interiorEdges.*abs(jmps);
            jumps  = sum(jmps(obj.t(1,:),obj.t(2,:)).^2,1);
        end
    end

    methods(Access = public)
        function DX = gradientMatrices(obj,grid)
            %%
            % * gradientMatrices IN: gridd OUT: double
            %
            % Method that computes matrix DX  , such that
            %
            % grad u =  (DX*u)'
            %
            % at the center of all elements.
            % DX is nElements x nPoints matrix. Note that this is
            % not an exact computation but an approximation with only
            % linear convergence.

            % TODO: If all lagrange classes have a gradientMatrices methods
            % introduce an abstract method in finiteElements

            % Note: We want to use makeJ, so it is not static...

            % The indices of the first and second  points in each
            % element
            idx1 = grid.t(1,:);
            idx2 = grid.t(2,:);

            % Compute Jacobi determinat
            J = obj.makeJ(grid);

            % FD Matrix
            p21 = 1./J;
            DX = sparse([1:grid.nElements,1:grid.nElements],...
                [idx1 idx2],[-p21 p21] ,...
                grid.nElements,grid.nPoints);
        end
    end
end
