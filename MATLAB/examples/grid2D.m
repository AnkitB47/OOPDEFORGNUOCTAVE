classdef(Abstract) grid2D < gridd
    ...
    % Future changes: grid2D become abstract
    % All methods that shape geometry become protected.
    % Idee: Shaping the domain shulb be done in a user class or by using
    % one of the example classes for "ususal" domains as rechtangles etc.

    % Changes:
    % 2017/11/03 Bugfix in jiggleMesh: getMeshQuality(obj) => obj.meshQuality
    % Rename method  square => rectangle.
    ...
    methods(Access = protected)
       % Basic geometries: free formed boundary, reactangle and circle.

        function freeGeometry(obj,pv,hmax)

            pv=[-0.4 -0.5;0.4 -0.2;0.4 -0.7;1.5 -0.4;0.9 0.1;
                 1.6 0.8;0.5 0.5;0.2 1;0.1 0.4;-0.7 0.7;-0.4 -0.5];
           [p,e,t]=distmesh2d(@dpoly,@huniform,0.1,[-1,-1; 2,1],pv,pv);
        ...
        function ellipse(obj,a1,a2,a3,a4,a5)
            ...
            if A<=0 || B <= 0
                ME = MException('ELLIPSE:wrongDefinedInput',...
                     'Error: A and B must be greater then zero.');
                ME.throwAsCaller();
            end
            % Prepare the boundary description function
            fd=@(p) (p(:,1)-a).^2/A^2+(p(:,2)-b).^2/B^2-1;
            % Call distmesh
            [p,e,t] = distmesh2d(fd,@huniform,hmax,[-A+a,-B+b;A+a,B+b],[]);
            obj.p = p';
            obj.e = e';
            obj.t = t';
            obj.t(4,:) = ones(1,size(obj.t,2));
        ...
        function[qval,gval,hval,rval] = boundCoefficients(p,b,varargin)
            ...
            if m == 0 % only Neumann BCs
                try
                    ...
                    ME.throwAsCaller();   %can also be done throw(ME)in place of 425,426
                end
                ...
                try
                    ...
                    if isscalar(rval)
                     ...
                     ME = MException(ME.identifier,ME.message);
                     ME.throwAsCaller(); %can also be done throw(ME)in place of 450,451
                end
            end
        end
    end
        ...
        function plot(obj,varargin)
            ...
            if (nargin == 1) || (mod(nargin-1,2) == 0)
                ...
                elseif length(z) == obj.nPoints
                    ...
                    ME = MException('GRID2D:PLOT:WRONGELEMENT',...
                         'Elements can only be P0,P1 or P2.');
                    ME.throwAsCaller();
                end
            end
            ...
            switch nargin
                ...
                case 3
                    switch varargin{1}
                        ...
                        otherwise
                            wrongFormat.throwAsCaller();
                    end
                otherwise
                    wrongNumberInputs.throwAsCaller();
            end
            try
                obj.refineRGB(toRefine);
            catch ME
                ME = MException('GRDI2D:REFINEMESH:INVALIDGRID',...
        ...
        function [bvalvec] = convCoefficientsMpt(obj,b)
            ...
            elseif isa(b1,'numeric')
                ...
                elseif length(b1)==obj.nElements
                    ...
                    ME = MException('ccoefficients:wrongSize',...
                         'wrong sized b(1)');
                    ME.throwAsCaller();
                end
            else
...
%                     max(fval)
        ...
        function indx = point2ElementIndex(obj,pt)
            ...
            if ~isvector(pt)
                ME = MException('GRID2D:NOVECTOR',...
                     'The argument must be the coordinates of a single point');
                ME.throwAsCaller();
            end
