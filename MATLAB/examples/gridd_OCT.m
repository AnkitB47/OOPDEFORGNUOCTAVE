...
classdef (Abstract) gridd < handle
    ...
    methods % For dependent properties. Do not define attributes here!
        ...
        function val = get.y(obj)
            ...
            elseif size(obj.p,1)<2
                ME = MException('GRIDD:NOSUCHPROP',...
                     [num2str(obj.spaceDimension),...
                     'D meshes have no property y.']);
                ME.throwAsCaller();
            else
                val = obj.p(2,:);
            end
        ...
        function val = get.nDirichletBoundaryPoints(obj)
            ...
            if isempty(obj.b)
                ME = MException('GRIDD:NOBOUNDARYCONDITION',...
    ...
    methods(Access = public)
        ...
        function makeBoundaryMatrix(obj,varargin)
            ...
            switch nargin
                case 1
                    ME = MException('GRIDD:WRONGBOUNDARYCOND',...
                ...
                otherwise
                    ...
                    if obj.nBoundarySegments == nargin-1
                        ...
                        ME = MException('GRIDD:WRONGBOUNDARYCOND',...
                             ...
                             ' matrix b and in geometry don''t match.']);
                        ME.throwAsCaller();
                    end
            end
                ...
                case 2   % obj and r given, tricky, second argument is r
                    ...
                    switch class(h)
                        ...
                        case 'char'
                            r = h;
                        otherwise
                            ME = MException('GRIDD:WRONGCLASS',...
                ...
                case 2
                    switch class(q)
                        ...
                        case 'char'
                            % ok
                        otherwise
                            ME = MException('GRIDD:WRONGCLASS','Wrong argument class, check it.');
                            ME.throwAsCaller();
                    end
        ...
        %   y2 = obj.point2Center(y1)
            ...
            elseif length(y)==obj.nPoints
                ME = MException('GRIDD:OPNOSENSE',...
                     'Size of y is already nPoints. Check your code!');
                ME.throwAsCaller();
            elseif ~length(y)==obj.nElements
                ME = MException('GRIDD:OPNOSENSE',...
                     'Size of y is not nElements.');
                ME.throwAsCaller();
            end
        ...
        function setSubdomainNumber(obj,indx,number,varargin)
            ...
            if isnumeric(indx)&&max(indx)>obj.nElements
                ME = MException('gridd:index',...
                     'Index must be in range of 1..nElements.');
                ME.throwAsCaller();
            end
        ...
        function rotateMesh(obj,varargin)
            ...
            if isempty(obj.p)
                % nothing to do
                ME = MException('GRIDD:SENSELESSACTION',...
                     'You try to rotate a empty grid.');
                ME.throwAsCaller();
            end
            switch obj.spaceDimension
                case 1
                    ME = MException('GRIDD:SENSELESSOPERATION',...
                         'It is not possible to rotate 1D objects.');
                    ME.throwAsCaller();
                case 2
                    switch length(varargin)
        ...
        function moveMesh(obj,varargin)
            ...
            switch obj.spaceDimension
                ...
                otherwise
                    ME = MException('GRIDD:UNKNOWNERROR','This should never hapen.');
                    ME.throwAsCaller();
            end
        end
        ...
        function scaleMesh(obj,scale)
            ...
            if scale>=1e-3 && scale <= 1e3
                ...
                ME = MException('GRIDD:WRONGPARAMETER',...
                     'Scale factor too small or to large.');
                ME.throwAsCaller();
            end
        end
            ...
            switch dim
                ...
                case {2,3}
                    ...
                    if length(u1)==size(p1,2)
                        ...
                        ME = MException('GRIDD:WRONGFORMAT',...
                             'Input has the wrong format, check it.');
                        ME.throwAsCaller();
                    end
                otherwise
                    % ME werfen...
                    ME = MException('PDE:NOTSUPPORTEDDIM',...
                         'PDE supports only FE in 1D-3D domains');
                    ME.throwAsCaller();
            end
...
% % %             u2(~obj.isPointInDomain(p2)) = NaN;
        ...
        function val = point2CenterB(obj,y)
            ...
            if isvector(y)&&length(y)~=obj.nPoints
                ME = MException('GRIDD:WRONGDATAFORMAT',...
                     'Data y must  have nPoints rows.');
                ME.throwAsCaller();
            else
            ...
            switch obj.spaceDimension
                case 1
                    ...
                    ME = MException('GRIDD:NOTVALID',...
                         'This method makes no sense for 1D grids');
                    ME.throwAsCaller();
                case 2
            ...
            switch nargin
                ...
                case 2
                    if isempty(intersect(boundarySegment,unique(obj.e(5,:))))
                        ME = MException('GRIDD:ARGUMENTSMISSMATCH',...
    ...
    methods(Access = public)
        function [sideLength,area] = sideLengthAndArea(obj)
            % Must be overridden in parent classes.
        end% okay for 1D 2D
    end
