            ...
            switch nargin
                ...
                case 1
                    if (isempty(obj.c)||isempty(obj.a)||isempty(obj.f))
                        ME = MException('PDE:COEFFIENCETS',...
                             'No coefficient functions set.');
                        ME.throwAsCaller();
                    end
                otherwise
                    ME = MException('PDE:WRONGNUMBERARGUMENTS',...
                         'The number of arguments must be zero or three.');
                    ME.throwAsCaller();
            end
                ...
                switch class(obj.c)
                    ...
                    case 'double' % dimension independent
                        if  isscalar(obj.c)
                            ...
                            ME = MException('PDE:WRONGCLASS',...
                                 ['For adaption, all data must be',...
                                 ' function_handles or scalar']);
                            ME.throwAsCaller() ;
                        end
                    case 'char'
                        try
