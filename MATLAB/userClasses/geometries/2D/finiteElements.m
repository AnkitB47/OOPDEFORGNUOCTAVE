...
classdef (Abstract) finiteElements < handle
    properties(Constant)
      % Universal error Objects.
      wrongNumberInputs = MException.getWrongNumberInputs();
      wrongNumberOutputs = MException.getWrongNumberOutputs();
      wrongFormat = MException.getWrongFormat();
      wrongClass = MException.getWrongClass();
      wrongSize = MException.getWrongSize();
    end
    ...
    methods (Access = public)
      %% Abtract methods with Access = public
        ...
        % OCTAVE did not know abstract classes
    ...
    methods(Static = true, Access = public)
        ...
        %  OCTAVE knows no static methods...

        function [idx0,idx1,idx2] = makeIndex(~,~,~)
        end
        %%
