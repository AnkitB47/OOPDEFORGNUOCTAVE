...
classdef (Abstract) finiteElements1D < finiteElements
    ...
    methods(Static, Access = public)
        ...
        function [Q,G,H,R] = assemb(gridObj)
            ...
            if m == 0 % case Robin
                Q(rE,rE) = eval(b(5:5+lengthq-1,2));
                G(rE) = eval(b(5+lengthq:5+lengthq+lengthg-1,2));
            else % case   Dirichlet BC
                lengthh = b(5,2);
                lengthr = b(6,2);
                H(2,rE) = eval(b(9:9+lengthh-1,2));
                R(2) = eval(b(9+lengthh:9+lengthh+lengthr-1,2));
            end
        end
        ...
        function [H] = periodic(gridObj)
            ...
            %
            % Implements periodic boudary conditions
            %
            % Call:
            %
            %       H = finiteElements1D.periodic(grid1D)
            %
            % Example:
            %
