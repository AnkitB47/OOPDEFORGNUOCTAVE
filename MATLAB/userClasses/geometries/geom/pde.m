classdef (Abstract) pde < handle
        ...
        function set.fem(obj,arg)
            if ismethod(obj,'checkfem')
                if obj.checkfem(arg)
                    ...
                    ME = MException('PDE:FORBIDDEN',...
                         'You cannot set fem property');
                    ME.throwAsCaller();
                end
            else
                obj.fem = arg;
    ...
    methods(Access = public)
        function initialize(obj,varargin)
            ...
            switch nargin
                ...
                case 6
                    ...
                    obj.initialized = true;
                otherwise
                    ME = MException(obj.wrongNumberArgumentsID,...
                         [obj.wrongNumberArgumentsStr,...
                         ' 5 (stationary case) or 6 (transient case)']);
                    ME.throwAsCaller();
            end
        end
        ...
        function solve(obj,solver)
            ...
            if ~ischar(solver)
                ME = MException(obj.wrongClassID,...
                     [obj.wrongClassStr,' a string']);
                ME.throwAsCaller();
            end
            switch solver
                case 'AMG'
                ...
                case 'LINEARGAUSS'
                    obj.linearGauss
                otherwise
                     ME = MException(obj.invalidOperationID,...
                ...
                switch varargin{k}
                    ...
                    case 'Robin'
                        if isnumeric(varargin{k+1})&&length(varargin{k+1})~=2
                           ME = MException('pde:setBoundaryCondition:missformedArgument' ,...
                                'The argument  must be a double or cell vector of lenght two.');
                           ME.throwAsCaller();
                        end
                        ...
                        l = l+1;
                    otherwise
                        ME = MException('pde:setBoundaryCondition:unknownArgument',...
            ...
            switch length(varargin)
                ...
                case 1
                    if strcmp(varargin{1},'lumped')
                        ...
                        ME = MException(obj.wrongClassID,...
                             [obj.wrongClassStr,...
                             ' string. Only allowed value is '' lumped''']);
                        ME.throwAsCaller();
                    end
                otherwise
                    ME = MException(obj.wrongNumberArgumentsID,...
                ...
                case 2
                    if isvector(arg)&& obj.grid.nPoints~=length(arg)
                        ...
                        if xdim~=obj.grid.nPoints&&...
                            ...
                            ME = MException(obj.wrongSizedArgumentID,...
        ...
        function refineMesh(obj,indx)
            ...
            switch nargin
                ...
                case 2
                    if dimSystem==3
                        ME = MException('PDE:NOTIMPLEMENTED',...
                             'Local refinement is still not implemented');
                        ME.throwAsCaller();
                    end
                    obj.grid.refineMesh(indx)
                otherwise
                    % wrong call, ME later
                    ME = MException('PDE:WRONNUMBERINPUTS',...
                         'Wrong numberof arguments.');
                    ME.throwAsCaller();
            end
    ...
    methods(Access=public)
        ...
        function ode15s(obj)
            ...
            if isempty(obj.y)||~obj.initialized
                ...
                ME = MException('PDE:NOTINITIALIZED',...
                     'The object is not initialized. Run pde.initialize befor call pde.solve');
                ME.throwAsCaller();
            ...
            if size(obj.D,1)~=size(obj.y,1)
                ME = MException('PDE:WRONGD',...
                     'Dimension of D do not fit the dimension of K, M, y etc.');
                ME.throwAsCaller();
            end
        ...
        function euleri(obj)
            ...
            function [iJ,y] = timestep(obj,k,iJ,dt)
                ...
                if StepFailed
                    ME = MException('newton:stepFailed',...
                    ...
                    switch obj.solverOptions.solver
                        ...
                        case 'pcg'
                            try
                                ...
                                if flag>0
                                    ME = MException('PDE:SOLVE:SOLVERFAILED',...
                        ...
                        otherwise
                            ME = MException('PDE:SOLVEUNKNOWNSOLVER',...
...
% % %                         StepFailed = false;
        ...
        function amg(obj)
            ...
            str = which('AMGSolver');
            if isempty(str)
                ME = MException('PDE:AMG:SOLVER_NOT_FOUND',...
