    ...
    methods
        function plotTimeSpace(obj,varargin)
            ...
            if size(obj.grid.p,1)>1
                ME = MException('plotUtilsTimeDependent:ONLY1D',...
                     'This method is only defined for 1D domains.');
                ME.throwAsCaller();
            end
            [p,indx] = sort(obj.grid.p);
            if any(size(obj.y)~=[length(p),length(obj.time)])
                ME = MException('PLOTUTILSTIMEDEPENDENT:DIMENSION',...
        ...
        function animation(obj,varargin)
            ...
            switch dim
                ...
                otherwise
                    ME = MException('plotUtilsTimeDependent:WRONGDIMENSION',...
                         'Domain must have dimension 1, 2, or 3.');
                    ME.throwAsCaller();
            end
        end
    end
